...
# dwl (Wayland)
#doas xbps-install -vy wmenu foot swaylock swaybg xorg-server-xwayland xdg-desktop-portal-wlr xdg-desktop-portal-gtk wl-clipboard wtype wlr-randr grim slurp swayidle wlopm qt5-wayland qt6-wayland zenity ImageMagick xf86-input-libinput libinput wayland wayland-protocols wlroots0.18 libxkbcommon fcft && doas xbps-install -vy libinput-devel wayland-devel wlroots0.18-devel libxkbcommon-devel fcft-devel libX11-devel libXft-devel libXinerama-devel && cd $HOME/stuffs/git/wl-color-picker && doas make clean install && cd $HOME/stuffs/git/dwl && doas make clean install && cd $HOME/stuffs/git/slstatus && doas make clean install && doas xbps-remove -RFfvy libinput-devel wayland-devel wlroots0.18-devel libxkbcommon-devel fcft-devel libX11-devel libXft-devel libXinerama-devel
